...
class Panel extends StatefulWidget {
  const Panel(this.header, this.child, this.controller, {Key key}) : super(key: key);
            ...
            Positioned.fill(
              top: MediaQuery.of(context).size.height * 0.2 +
                  ((MediaQuery.of(context).size.height * 0.8) -
                      (MediaQuery.of(context).size.height * 0.8) * widget.controller.value),
              child: InkWell(
                onTap: () {
                    ...
                    children: <Widget>[
                          ...
                          onScaleUpdate: (ScaleUpdateDetails sud) {
                            if (_startPoint.dy > sud.focalPoint.dy) {
                              widget.controller.value =
                                  0.2 + max(0, _startPoint.dy - sud.focalPoint.dy) / MediaQuery.of(context).size.height;
                            } else {
                              widget.controller.value =
                                  1.0 - max(0, sud.focalPoint.dy - _startPoint.dy) / MediaQuery.of(context).size.height;
                            }
                          },
                      ...
                      Positioned.fill(
                        ...
                        child: IgnorePointer(
                          ...
                          child: Opacity(
                            opacity:
                                widget.controller.value < 0.2 ? 0.0 : map(widget.controller.value, 0.2, 1.0, 0.0, 1.0),
                            child: widget.child,
                          ),
                        ),
                      )
...
enum PanelState { closed, collapsed, expanded }
