...
class GameTicker extends StatelessWidget {
  const GameTicker({Key key}) : super(key: key);
...
class _RoundTicker extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(const Offset(50.0, -50.0), 20.0, p);

    p.color = Colors.grey;
    canvas.drawArc(
        Rect.fromCircle(center: const Offset(50.0, -50.0), radius: 20.0), -pi * 0.5, pi * 2 * progress, true, p);

    p.color = Colors.grey.shade600;
    p.style = PaintingStyle.stroke;
    p.strokeWidth = 1.0;
    canvas.drawArc(Rect.fromCircle(center: const Offset(50.0, -50.0), radius: 20.0), -pi * 0.5, pi * 2, false, p);
  }
