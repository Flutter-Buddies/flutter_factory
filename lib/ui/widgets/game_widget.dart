...
class _GameWidgetState extends State<GameWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onTapUp: (TapUpDetails tud) =>
          _bloc.onTapUp(tud, Theme.of(context), DynamicTheme.of(context).data, Scaffold.of(context).showSnackBar),
      child: StreamBuilder<GameUpdate>(
        stream: _bloc.gameUpdate,
        builder: (BuildContext context, AsyncSnapshot<GameUpdate> snapshot) {
          if (!_bloc.moneyManager.hasClaimedCredit && _bloc.moneyManager.idleCredit != 0) {
  ...
  void showIdleDialog() {
    _bloc.moneyManager.hasClaimedCredit = true;
...
class GamePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    bloc.selectedTiles.forEach((Coordinates c) {
      ...
      if (_equipment == null) {
        canvas.drawRect(
            ...
            Paint()
              ..color = bloc.moneyManager.canPurchaseEquipment(bloc.buildSelectedEquipmentType,
                      bulkBuy: (bloc.selectedTiles.indexOf(c) + 1))
                  ? theme.selectedTileColor
                  : theme.negativeActionButtonColor.withOpacity(0.4));
        return;
      }
    ...
    bloc.equipment
        .where(
            (FactoryEquipmentModel fe) => fe is UndergroundPortal && fe.connectingPortal != null && fe.distance != null)
        .map<UndergroundPortal>((FactoryEquipmentModel fem) => fem)
        .forEach((UndergroundPortal up) {
      ...
      if (up.coordinates.x == up.connectingPortal.x) {
        ...
        for (int i = 0; i < (up.distance + 1); i++) {
          canvas.drawRect(
              Rect.fromCircle(
                  center:
                      Offset(up.coordinates.x * bloc.cubeSize, (up.coordinates.y + (_goUp ? -i : i)) * bloc.cubeSize),
        ...
        for (int i = 0; i < (up.distance + 1); i++) {
          canvas.drawRect(
              Rect.fromCircle(
                  center: Offset(
                      (up.coordinates.x + (_goRight ? -i : i)) * bloc.cubeSize, up.coordinates.y * bloc.cubeSize),
                  radius: bloc.cubeSize / 2),
              Paint()..color = up.lineColor.withOpacity(0.25));
        }
      }
    ...
    if (bloc.movingEquipment != null) {
      ...
      bloc.movingEquipment.forEach((FactoryEquipmentModel fem) {
        ...
        canvas.clipRect(Rect.fromCircle(
            center: Offset(fem.coordinates.x * bloc.cubeSize, fem.coordinates.y * bloc.cubeSize),
            radius: bloc.cubeSize / 2));
        fem.drawTrack(theme, Offset(fem.coordinates.x * bloc.cubeSize, fem.coordinates.y * bloc.cubeSize), canvas,
            bloc.cubeSize, bloc.progress);
        fem.drawEquipment(theme, Offset(fem.coordinates.x * bloc.cubeSize, fem.coordinates.y * bloc.cubeSize), canvas,
            bloc.cubeSize, bloc.progress);
        canvas.drawRect(
            Rect.fromCircle(
                center: Offset(fem.coordinates.x * bloc.cubeSize, fem.coordinates.y * bloc.cubeSize),
                radius: bloc.cubeSize / 2),
            Paint()
              ..color = _inLimits ? theme.floorColor.withOpacity(0.5) : theme.machineInActiveColor.withOpacity(0.6));
        if (_inLimits) {
          canvas.drawRect(
              Rect.fromCircle(
                  center: Offset(fem.coordinates.x * bloc.cubeSize, fem.coordinates.y * bloc.cubeSize),
              ...
              Paint()
                ..color = _canAfford
                    ? theme.selectedTileColor.withOpacity(0.5)
