...
class WaterHeater extends FactoryMaterialModel{
  WaterHeater.fromOffset(Offset o) : super(o.dx, o.dy, 13000.0, FactoryMaterialType.waterHeater, state: FactoryMaterialState.crafted);

  WaterHeater.custom({double x, double y, double value, double size = 8.0, FactoryMaterialState state = FactoryMaterialState.crafted, double rotation, double offsetX, double offsetY}) :
      super.custom(x: x, y: y, value: value, type: FactoryMaterialType.waterHeater, size: size, state: state, rotation: rotation, offsetX: offsetX, offsetY: offsetY);
  ...
  void drawMaterial(Offset offset, Canvas canvas, double progress, {double opacity = 1.0}){
    ...
    _p.style = PaintingStyle.fill;
    canvas.drawCircle(
      Offset(_size * 0.15, _size * 0.86),
      _size * 0.02,
      _p
    );
    _p.color = Colors.white;
    canvas.drawCircle(
      Offset(_size * 0.05, _size * 0.9),
      _size * 0.05,
      _p
    );
