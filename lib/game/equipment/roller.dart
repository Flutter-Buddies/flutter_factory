...
class Roller extends FactoryEquipmentModel {
  Roller(Coordinates coordinates, Direction direction, {int rollerTickDuration = 1})
      : super(coordinates, direction, EquipmentType.roller, tickDuration: rollerTickDuration);
  ...
  void drawTrack(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    if (direction == Direction.east || direction == Direction.west) {
      canvas.drawRect(Rect.fromPoints(Offset(size / 3, size / 3), Offset(-size / 3, -size / 3)),
          Paint()..color = theme.rollersColor);
    } else {
      canvas.drawRect(Rect.fromPoints(Offset(size / 3, size / 3), Offset(-size / 3, -size / 3)),
          Paint()..color = theme.rollersColor);
    }
      ...
      switch (d) {
        case Direction.west:
          for (int i = 0; i < 4; i++) {
            ...
            canvas.drawLine(
                Offset(_xOffset - size / 3 - size / 3 + size / 2, size / 3),
                Offset(_xOffset - size / 3 - size / 3 + size / 2, -size / 3),
        ...
        case Direction.south:
          for (int i = 0; i < 4; i++) {
            final double _yOffset =
                ((size / 6) * i + (entry ? progress : -progress) * size) % (size / 1.5) - size / 3 / 2;
            canvas.drawLine(
                Offset(size / 3, _yOffset - size / 3 - size / 3 + size / 2),
                Offset(-size / 3, _yOffset - size / 3 - size / 3 + size / 2),
                Paint()..color = theme.rollerDividersColor);
          }
          break;
  ...
  FactoryEquipmentModel copyWith({Coordinates coordinates, Direction direction, int tickDuration}) {
    return Roller(coordinates ?? this.coordinates, direction ?? this.direction,
