...
class HydraulicPress extends FactoryEquipmentModel {
  HydraulicPress(Coordinates coordinates, Direction direction, {this.pressCapacity = 1, int tickDuration = 1})
      : super(coordinates, direction, EquipmentType.hydraulic_press, tickDuration: tickDuration);
  ...
  HydraulicPress copyWith({Coordinates coordinates, Direction direction, int pressCapacity}) {
    return HydraulicPress(coordinates ?? this.coordinates, direction ?? this.direction,
  ...
  void _processMaterial() {
    ...
    void _changeState(FactoryMaterialModel fmm) {
      fmm.changeState(FactoryMaterialState.plate);
    }
  ...
  void drawEquipment(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    final Path _machineLegs = Path();

    if (direction == Direction.east || direction == Direction.west) {
      _machineLegs.addRRect(RRect.fromRectAndCorners(
          Rect.fromPoints(Offset(size / 2.2, size / 2.2), Offset(-size / 2.2, size / 3.0)),
          bottomLeft: Radius.circular(size / 2.2 / 2),
          bottomRight: Radius.circular(size / 2.2 / 2)));
      _machineLegs.addRRect(RRect.fromRectAndCorners(
          Rect.fromPoints(Offset(size / 2.2, -size / 2.2), Offset(-size / 2.2, -size / 3.0)),
          topLeft: Radius.circular(size / 2.2 / 2),
          topRight: Radius.circular(size / 2.2 / 2)));
    } else {
      _machineLegs.addRRect(RRect.fromRectAndCorners(
          Rect.fromPoints(Offset(size / 2.2, size / 2.2), Offset(size / 3.0, -size / 2.2)),
          topRight: Radius.circular(size / 2.2 / 2),
          bottomRight: Radius.circular(size / 2.2 / 2)));
      _machineLegs.addRRect(RRect.fromRectAndCorners(
          Rect.fromPoints(Offset(-size / 2.2, size / 2.2), Offset(-size / 3.0, -size / 2.2)),
...
//    _path.addRRect(RRect.fromRectAndCorners(Rect.fromPoints(Offset(size / 2.2, size / 2.2), Offset(-size / 2.2, -size / 2.2))));
    ...
    final double _change = Curves.easeInCubic.transform(_machineProgress) * 3;
    canvas.drawRRect(
        RRect.fromRectAndRadius(
                Rect.fromPoints(Offset(size / 2.5, size / 2.5), Offset(-size / 2.5, -size / 2.5)).deflate(_change),
                Radius.circular(size / 2.5 / 2))
            .deflate(_change),
        Paint()..color = theme.machineWarningColor);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
                Rect.fromPoints(Offset(size / 2.7, size / 2.7), Offset(-size / 2.7, -size / 2.7)).deflate(_change),
                Radius.circular(size / 2.7 / 2))
            .deflate(_change),
        Paint()..color = theme.machinePrimaryLightColor);

    canvas.drawRRect(
        RRect.fromRectAndRadius(Rect.fromPoints(Offset(size / 2.0, size / 2.0), Offset(-size / 2.0, -size / 2.0)),
            Radius.circular(size / 2.5 / 2)),
        Paint()..color = Colors.white24);

    final Path _machineTop = Path();
    if (direction == Direction.east || direction == Direction.west) {
  ...
  Map<String, dynamic> toMap() {
    ...
    _map.addAll(<String, dynamic>{'press_capacity': pressCapacity});
    return _map;
  }
}
