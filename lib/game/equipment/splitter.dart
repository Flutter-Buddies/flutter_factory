...
class Splitter extends FactoryEquipmentModel {
  Splitter(Coordinates coordinates, Direction equipmentDirection, this.directions)
      : _direction = equipmentDirection,
        super(coordinates, equipmentDirection, EquipmentType.splitter);
  ...
  void drawTrack(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    _candidates.forEach(_drawSplitter);
  ...
  void paintInfo(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    void _addDirectionText(Direction d) {
      _paragraphBuilder.addText(
          '${directionToString(d)} x ${directions.where((Direction dd) => dd == d).length ?? 0}${d.index == 1 ? '\n' : '  '}');
    }

    Direction.values.forEach(_addDirectionText);
  ...
  Map<String, dynamic> toMap() {
    ...
    _map.addAll(<String, dynamic>{'splitter_directions': directions.map((Direction d) => d.index).toList()});
    return _map;
  }
}
