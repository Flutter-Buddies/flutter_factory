...
class RotatingFreeRoller extends FactoryEquipmentModel {
  RotatingFreeRoller(Coordinates coordinates, Direction equipmentDirection, {int tickDuration, this.rotation = 1})
      : super(coordinates, equipmentDirection, EquipmentType.rotatingFreeRoller, tickDuration: tickDuration);
...
//      }
//    }
    canvas.drawCircle(Offset.zero, _size * 0.4, Paint()..color = theme.rollersColor);

    canvas.drawLine(
        Offset(cos((rotation == 1 ? progress : -progress) * pi * 2) * _size * 0.1,
            sin((rotation == 1 ? progress : -progress) * pi * 2) * _size * 0.1),
        Offset(cos((rotation == 1 ? progress : -progress) * pi * 2) * _size * 0.4,
            sin((rotation == 1 ? progress : -progress) * pi * 2) * _size * 0.4),
        Paint()
          ..color = theme.machinePrimaryLightColor
          ..strokeWidth = 1.0);
  ...
  void drawMaterial(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    void _drawRotatingMaterial(FactoryMaterialModel fm) {
      double _moveX = 0.0;
      double _moveY = 0.0;
    ...
    objects.forEach(_drawRotatingMaterial);
  }
