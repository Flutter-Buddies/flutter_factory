...
class Melter extends FactoryEquipmentModel {
  Melter(Coordinates coordinates, Direction direction, {this.meltCapacity = 1, int tickDuration = 1})
      : super(coordinates, direction, EquipmentType.melter, tickDuration: tickDuration);
  ...
  Melter copyWith({Coordinates coordinates, Direction direction, int meltCapacity}) {
    return Melter(coordinates ?? this.coordinates, direction ?? this.direction,
  ...
  void _processMaterial() {
    ...
    void _changeState(FactoryMaterialModel fmm) {
      fmm.changeState(FactoryMaterialState.fluid);
    }
  ...
  void drawEquipment(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    canvas.drawRRect(
        RRect.fromRectAndRadius(Rect.fromPoints(Offset(size / 2.6, size / 2.6), Offset(-size / 2.6, -size / 2.6)),
            Radius.circular(size / 2.6 / 2)),
        Paint()..color = Color.lerp(theme.machineAccentLightColor, theme.melterActiveColor, _machineProgress));
  ...
  Map<String, dynamic> toMap() {
    ...
    _map.addAll(<String, dynamic>{'melt_capacity': meltCapacity});
    return _map;
  }
}
