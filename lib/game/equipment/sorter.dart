...
class Sorter extends FactoryEquipmentModel {
  Sorter(Coordinates coordinates, Direction equipmentDirection, this.directions)
      : super(coordinates, equipmentDirection, EquipmentType.sorter);
  ...
  List<FactoryMaterialModel> tick() {
    ...
    _fm.map((FactoryMaterialModel fm) {
      final FactoryRecipeMaterialType _frmt = directions.keys.firstWhere(
          (FactoryRecipeMaterialType frmt) => frmt.materialType == fm.type && frmt.state == fm.state,
          orElse: () => null);
  ...
  void drawTrack(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    void _drawSplitter(Direction d) {
      if (d == direction) {
        return;
      } else {
  ...
  void paintInfo(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    final ParagraphBuilder _paragraphBuilder = ParagraphBuilder(ParagraphStyle(textAlign: TextAlign.start));
    _paragraphBuilder.pushStyle(TextStyle(color: Colors.white, fontSize: 6.0, fontWeight: FontWeight.w500));

    void _addDirection(Direction d) {
      _paragraphBuilder.addText('${directionToString(d)}\n');
    }

    Direction.values.forEach(_addDirection);
    ...
    void _sorterPaintInfo(Direction d) {
      final List<FactoryRecipeMaterialType> _materials = directions.keys.where((FactoryRecipeMaterialType fmrt) {
        return directions[fmrt] == d;
      }).toList();

      void _drawMaterial(FactoryRecipeMaterialType fmt) {
        canvas.save();
        canvas.translate(
            -size * 0.45 + (size * 0.5 * (_materials.indexOf(fmt) / _materials.length)), size * 0.24 * (d.index + 1));
        canvas.scale(0.5);

        FactoryMaterialModel.getFromType(fmt.materialType)
          ..state = fmt.state
          ..drawMaterial(Offset.zero, canvas, 0.0);
    ...
    Direction.values.forEach(_sorterPaintInfo);
  ...
  void drawEquipment(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    if (Direction.values[(direction.index - 2) % Direction.values.length] == Direction.north ||
        objects.indexWhere((FactoryMaterialModel fm) => _materialToDirection(fm, Direction.south)) != -1 ||
        Direction.values[(direction.index - 2) % Direction.values.length] == Direction.north) {
    ...
    if (Direction.values[(direction.index - 2) % Direction.values.length] == Direction.south ||
        objects.indexWhere((FactoryMaterialModel fm) => _materialToDirection(fm, Direction.north)) != -1 ||
        Direction.values[(direction.index - 2) % Direction.values.length] == Direction.south) {
      _gatesPaint.color = theme.machineActiveColor;
    } else {
      _gatesPaint.color = theme.machineAccentLightColor;
    }
    ...
    if (Direction.values[(direction.index - 2) % Direction.values.length] == Direction.east ||
        objects.indexWhere((FactoryMaterialModel fm) => _materialToDirection(fm, Direction.west)) != -1 ||
        Direction.values[(direction.index - 2) % Direction.values.length] == Direction.east) {
      _gatesPaint.color = theme.machineActiveColor;
    } else {
      _gatesPaint.color = theme.machineAccentLightColor;
    }
    ...
    if (Direction.values[(direction.index - 2) % Direction.values.length] == Direction.west ||
        objects.indexWhere((FactoryMaterialModel fm) => _materialToDirection(fm, Direction.east)) != -1 ||
        Direction.values[(direction.index - 2) % Direction.values.length] == Direction.west) {
      _gatesPaint.color = theme.machineActiveColor;
    } else {
      _gatesPaint.color = theme.machineAccentLightColor;
    }
  ...
  FactoryEquipmentModel copyWith(
      {Coordinates coordinates, Direction direction, Map<FactoryRecipeMaterialType, Direction> directions}) {
    return Sorter(
        coordinates ?? this.coordinates,
        direction ?? this.direction,
        directions ?? <FactoryRecipeMaterialType, Direction>{}
          ..addAll(this.directions));
  }
  ...
  Map<String, dynamic> toMap() {
    ...
    _map.addAll(<String, dynamic>{
      'sorter_directions': directions.keys
          .map((FactoryRecipeMaterialType type) => <String, int>{
