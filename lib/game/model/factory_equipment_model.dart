...
//    print('Input: ${equipmentTypeToString(type)} - ${inputDirections.keys.fold('', (String s, Direction d) => s += directionToString(d))}');
  ...
  void drawSplitter(GameTheme theme, Direction d, Canvas canvas, double size, double progress, {bool entry = false}) {
    ...
    switch (d) {
      case Direction.west:
        canvas.drawRect(
            Rect.fromCircle(center: Offset(-size / 3, 0.0), radius: size / 3), Paint()..color = theme.rollersColor);
        for (int i = 0; i < 4; i++) {
          final double _xOffset =
              ((size / 6) * i + (entry ? progress : -progress) * size) % (size / 1.5) - size / 3 / 2;
          canvas.drawLine(Offset(_xOffset - size / 3 - size / 3 + size / 6, size / 3),
              Offset(_xOffset - size / 3 - size / 3 + size / 6, -size / 3), Paint()..color = theme.rollerDividersColor);
        }
        break;
      case Direction.east:
        canvas.drawRect(
      ...
      case Direction.south:
        canvas.drawRect(
            Rect.fromCircle(center: Offset(0.0, -size / 3), radius: size / 3), Paint()..color = theme.rollersColor);
        for (int i = 0; i < 4; i++) {
          final double _yOffset =
              ((size / 6) * i + (entry ? progress : -progress) * size) % (size / 1.5) - size / 3 / 2;
          canvas.drawLine(Offset(size / 3, _yOffset - size / 3 - size / 3 + size / 6),
              Offset(-size / 3, _yOffset - size / 3 - size / 3 + size / 6), Paint()..color = theme.rollerDividersColor);
        }
        break;
      case Direction.north:
        canvas.drawRect(
    ...
    switch (d) {
      case Direction.east:
        for (int i = 0; i < 6; i++) {
          ...
          canvas.drawLine(
              Offset(size / 2 + size / 6 - _xOffset, size / 3),
              Offset(size / 2 + size / 6 - _xOffset, -size / 3),
              Paint()
                ..color = theme.rollersColor
                ..strokeWidth = 2.2);
        }
        break;
      case Direction.west:
      ...
      case Direction.south:
        for (int i = 0; i < 6; i++) {
          final double _yOffset = i * (size / 8);
          canvas.drawLine(
              Offset(size / 3, _yOffset - size / 2 - size / 6),
              Offset(-size / 3, _yOffset - size / 2 - size / 6),
              Paint()
                ..color = theme.rollersColor
                ..strokeWidth = 2.2);
      ...
      case Direction.north:
        for (int i = 0; i < 6; i++) {
          final double _yOffset = i * (size / 8);
          canvas.drawLine(
              Offset(size / 3, size / 2 + size / 6 - _yOffset),
              Offset(-size / 3, size / 2 + size / 6 - _yOffset),
              Paint()
                ..color = theme.rollersColor
                ..strokeWidth = 2.2);
  ...
  void paintInfo(GameTheme theme, Offset offset, Canvas canvas, double size, double progress) {
    ...
    switch (direction) {
      case Direction.east:
        {
          canvas.drawLine(Offset(x - size / 2.2, y), Offset(x + size / 2.2, y), _p);
          canvas.drawLine(Offset(x + size / 2.2, y), Offset(x, y + size / 2.5), _p);
          canvas.drawLine(Offset(x + size / 2.2, y), Offset(x, y - size / 2.5), _p);
          break;
        }
      case Direction.west:
      ...
      case Direction.south:
        {
          canvas.drawLine(Offset(x, y - size / 2.2), Offset(x, y + size / 2.2), _p);
          canvas.drawLine(Offset(x - size / 2.5, y), Offset(x, y - size / 2.2), _p);
          canvas.drawLine(Offset(x + size / 2.5, y), Offset(x, y - size / 2.2), _p);
          break;
        }
      case Direction.north:
  ...
  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      ...
      'material': objects
          .getRange(max(0, objects.length - 50), objects.length)
          .map((FactoryMaterialModel fmm) => fmm.toMap())
          .toList()
    };
  }
}
...
String equipmentDescriptionFromType(EquipmentType type) {
  switch (type) {
    case EquipmentType.dispenser:
      return 'Dispenses new raw material that can be processed, crafted or sold.';
    case EquipmentType.roller:
      return 'Moves material in selected direction.';
    case EquipmentType.freeRoller:
      return 'Moves material but it won\'t change it\'s direction.';
    case EquipmentType.crafter:
      return 'Crafter can craft new more complicated materials.';
    case EquipmentType.splitter:
      return 'Splits incoming materials to all selected directions equally. Multiples of same direction can be set.';
    case EquipmentType.sorter:
      return 'It can move specific material in any direction, it has \'default\' exit where any other material will exit.';
    case EquipmentType.seller:
      return 'Sells material for profit.';
    case EquipmentType.hydraulic_press:
      return 'Press raw material to make plates.';
    case EquipmentType.wire_bender:
      return 'Bend raw material to make springs.';
    case EquipmentType.cutter:
      return 'Cut raw material to make gears.';
    case EquipmentType.melter:
      return 'Melt raw material to get liquid.';
    case EquipmentType.rotatingFreeRoller:
      return 'Will rotate any material that comes in for 90 deg';
    case EquipmentType.portal:
      return 'Will transfer material underground, automatically connects to other portal enterance';
  }
