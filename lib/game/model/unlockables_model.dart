...
class GameItems {
  ...
  int _getMachineCost(EquipmentType type) {
    switch (type) {
  ...
  int _getMachineUnlockCost(EquipmentType type) {
    switch (type) {
      case EquipmentType.roller:
        return 5000;
      case EquipmentType.freeRoller:
  ...
  int _getRecipeCost(FactoryMaterialType type) {
    switch (type) {
...
class UnLockableRecipes {
  ...
  Map<String, dynamic> toMap() {
    return <String, dynamic>{'recepie': recipeType.index, 'unlocked': isUnlocked, 'unlock_cost': unlockCost};
  }
}
